...
TitlebarLikeWindows10::TitlebarLikeWindows10(Frameless *fl,const int &buttonFixeWidth,QWidget *parent) :
    QWidget(parent),
...
QToolButton{
    ...
    connect(fl,&Frameless::windowStateChanged,this,[=](const Qt::WindowStates &state){
        this->load_round4(!state.testFlag(Qt::WindowMaximized));
    });
}

TitlebarLikeWindows10::~TitlebarLikeWindows10(){
    delete ui;
    ____<<"~TitlebarLikeWindows10";
}
...
void TitlebarLikeWindows10::draw(ToolButton_righttop_windows10* button){
    ...
    if (button->type==RIGHTTOP_TYPE::__type__::Minimize){
        if (is_not_activate){
            ...
            border_rect.setTop(0); border_rect.setBottom(height-1);
            if(button->isDown()){
